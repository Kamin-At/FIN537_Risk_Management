...
cov = matrix(c(0.000332, 0.000156,	0.000176,	-0.000314,	-0.000261,	-0.000209,
               0.000156,	0.000178,	0.000187,	-0.000170,	-0.000258,	-0.000221,
               0.000176,	0.000187,	0.000289,	-0.000155,	-0.000263,	-0.000294,
               -0.000314,	-0.000170,	-0.000155,	0.000996,	0.000750,	0.000597,
               -0.000261,	-0.000258,	-0.000263,	0.000750,	0.000863,	0.000687,
               -0.000209,	-0.000221,	-0.000294,	0.000597,	0.000687,	0.000715
               ), nrow = 6, ncol = 6)

n=1000
set.seed(137)
sim_log_ret = mvrnorm(n, mu=mu, Sigma = cov)
colnames(sim_log_ret) <- c("log_ret_xle", "log_ret_xlf", 'log_ret_xly',"log_vol_xle", "log_vol_xlf", 'log_vol_xly')

new_t = 60/252
PandL_values = rep(-1, nrow(df))

for (i in 1:nrow(sim_log_ret)){
  tmp_new_xle = td_xle * exp(sim_log_ret[i, "log_ret_xle"])
  tmp_new_xlf = td_xlf * exp(sim_log_ret[i, "log_ret_xlf"])
  tmp_new_xly = td_xly * exp(sim_log_ret[i, "log_ret_xly"])
  
  tmp_new_vol_xle = td_xle_sigma * exp(sim_log_ret[i, "log_vol_xle"])
  tmp_new_vol_xlf = td_xlf_sigma * exp(sim_log_ret[i, "log_vol_xlf"])
  tmp_new_vol_xly = td_xly_sigma * exp(sim_log_ret[i, "log_vol_xly"])
...
cat("CVaR @ 5% =", -mean(PandL_values[which(PandL_values <= quantile(PandL_values, 0.05))]))